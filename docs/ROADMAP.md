...
Chapters 00–09 are published and were replayed from their implementation tags during the current pause debrief. A chapter becomes public after its code checkpoint, exact command, observable result, provenance, historical and current source comparisons, and responsive browser audit are complete. Passing a chapter proves that fixture, not production or platform support.

**Current recommendation: no-go on Chapter 10 until GTK ownership/close behavior, capture-only timeout, binding strategy, graphics capability probing, native CI, and pixel-region verification are resolved.**
...
11. **First GPU rectangle** — runtime Surface owns GtkGLArea; clean close; OpenGL capabilities recorded; one known rectangle; before/after regional pixel verification
