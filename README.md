...
The published path now contains the first Zig process, App ownership, entry routing, runtime/Surface split, a measured ordinary-pipes limitation, the first Linux PTY session, and persistent Termio byte ownership. Later chapters are written only after the previous checkpoint is learned and validated.
