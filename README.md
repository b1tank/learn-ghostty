...
The published path now contains the first Zig process, App ownership, entry routing, runtime/Surface split, a measured ordinary-pipes limitation, the first Linux PTY session, persistent Termio byte ownership, an incremental VT parser, durable terminal state, and the first real native GTK window. Later chapters are written only after the previous checkpoint is learned and validated.
